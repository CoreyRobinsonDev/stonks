...
const Sell = () => {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    setIsPending(true);
    axios.post("https://stonks-crd.herokuapp.com/stocks/sell", {
      user_id: user?.id,
  ...
  return <section className={SellCSS.container}>
    ...
    <form className={SellCSS.form} onSubmit={(e) => handleSubmit(e)}>
      ...
      <small className={`${SellCSS.message} ${message === "Invalid ticker symbol" || message === "Insufficient shares" || message === "Shares must be in positive integer amounts" ? SellCSS.alert : ""}`}>{isPending ? <LoadingDots/> : message}</small>
